...
class SawyerReachPushPickPlaceEnv(SawyerXYZEnv):
    ...
    def model_name(self):
        return get_asset_full_path(
            'sawyer_xyz/sawyer_reach_push_pick_and_place_complete.xml')
    ...
    def _get_obs_dict(self):
        hand = self.get_endeff_pos()
        #objPos = self.data.get_geom_xpos('objGeom')
        flat_obs = hand  #np.concatenate((hand, objPos))
        return dict(
    ...
    def adjust_initObjPos(self, orig_init_pos):
        ...
        diff = self.get_body_com('obj')[:2] - self.data.get_geom_xpos(
            'objGeom')[:2]
    ...
    def reset_model(self):
        ...
        # ]
        # Melissa: Removed other tasks rewards, this will break other tasks
        self.target_rewards = [1000, 1000 * self.maxReachDist + 1000 * 2, 1000]
        if self.task_type == 'reach':
            idx = 1
        elif self.task_type == 'push':
            idx = 2
    ...
    def compute_reward(self, actions, obs, mode='general', task_type='reach'):
        ...
        def compute_reward_push(actions, obs, mode):
            ...
            return [
                reward, reachRew, reachDist, pushRew, pushDist, None, None,
                None
            ]
        ...
        def compute_reward_pick_place(actions, obs, mode):
            ...
            def objDropped():
                return (objPos[2] < (self.objHeight + 0.005)) and (
                    placingDist > 0.02) and (reachDist > 0.02)
            ...
            def orig_pickReward():
                ...
                elif (reachDist < 0.1) and (objPos[2] >
                                            (self.objHeight + 0.005)):
            ...
            def placeReward():
                ...
                if mode == 'general':
                    ...
                    cond = self.pickCompleted and (reachDist <
                                                   0.1) and not (objDropped())
                if cond:
                    placeRew = 1000 * (self.maxPlacingDist -
                                       placingDist) + c1 * (
